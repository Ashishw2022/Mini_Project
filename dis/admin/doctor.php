...
<body>
  ...
  <a href="doctor.php"class="icon-a active"><i class="fas fa-user-md"></i> &nbsp;&nbsp;Doctors</a>
...
<div id="main">
    <?php
    $query = "SELECT d_id,FirstName,LastName,email,pass,speciali,d.loginid,l.sta,phonenumber,address,education,experience FROM d_reg d,login l WHERE d.loginid=l.loginid";
        ...
        if(mysqli_num_rows($qu_run)>0) 
        {
            while($row = mysqli_fetch_assoc($qu_run))
            {
              ?>
                ...
                <td><?php 
                    if($row['sta']==1){
                    echo '<p><a href="sta.php?id='.$row['loginid'].'&status=0" type="submit"  class="btn">Enable</a></p></td>';
                 
                    }
                  else{
                    echo '<p><a href="sta.php?id='.$row['loginid'].'&status=1" type="submit" id="dangbtn" class="btn ">Disable</a></p></td>';
                      }?>
               <!-- <td><a href="d_edit.php?id=<?php echo $row['loginid']; ?>" type="submit" class="btn btn-primary">Edit</a></td> -->
